	...
	for token_list in data:
		...
		for t in token_list:
			...
			if chunk == True:
				...
				d_t[key] = 1
			if w2v == True:
				if t[0] in w2v_model.wv:
					word_vec = w2v_model.wv[t[0]]
					for i,w in enumerate(word_vec[:50]):
						key = 'w2v_'+str(i)
						d_t[key] = w
			ner.append(t[3])
			id.append(d_t)
...
def evaluate_combinations(train_data, test_data):
	...
	argnames = ['caps', 'pos', 'chunk', 'context','w2v']
	cols = ['f1', 'n_params'] + argnames
	df = pd.DataFrame(columns=cols)
	combs = product(caps, pos, chunk, context,w2v)
	for i,c in enumerate(combs):
		dicts, labels = make_feature_dicts(train_data,w2v_model,True,*c)
		test_dicts, test_labels = make_feature_dicts(test_data,w2v_model, True,*c)
		vec = DictVectorizer()
...
if __name__ == '__main__':
	...
	w2v_model = word2vec.Word2Vec(brown_sents, size=50, window=5, min_count=5)
